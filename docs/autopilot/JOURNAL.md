...
## 2026-09-21 22:18 — autopilot/map-c
Done: #65 (extractor #2: slope band 100→60) merged and merged in. #64 (draft) now carries corrections M-020–M-084 (11 terrain, 1 coast, 13 slope/up, 4 settlement, 32 unfinished railroad — the Matruh–Capuzzo–Tobruk line is printed blue, 4 false tracks from map titles and the frontier wire), 16 places, sample 0/50, map_diff skipping cross-sheet keys, EXTRACTION entry current. 98 resolutions this PR (65 corrections + 33 agrees in build/map/diff-C-resolved.md). Diff 470 → 206 rows, ~170 unresolved.
In flight: nothing half-written. `build/map/diff-C.md` regenerates with `map_diff.py C`; crops via `map_sample.py C --hex`; montage recipe: debug crop of the hexside (map_extract --debug around the midpoint) beside a 220×180 scan crop at map_sample.scan_centre.
Next: 52 more resolutions fit under the cap on #64 — take the 78 "- → track" rows first (real crossings the 1 px dashes hide; each needs an add-hexside correction citing the scan), then the 15 "unfinished-railroad → track,unfinished-railroad" (check whether a track runs beside the blue line). At the cap: place the Giarabub Oasis palm hexes, run the gate line, decide on `gh pr ready 64` per the spec's 150 rule, and note the remaining count in PROGRESS. Then Task 15.
Blocked: none. For Brian: `up` at C0323|C0423; C4905|C4906 (coast road or track).
