...
| `site/` | VitePress site that renders `rules/` and `data/`, plus the published *Learn* primer (`site/learn.md`, generated by `tools/learn_page.py --site`) | MIT |
| `docs/` | Designs, plans; the standalone primer build (`tools/learn_page.py`, with chart crops and Map C, git-ignored) stays local | CC-BY-SA-4.0 |
